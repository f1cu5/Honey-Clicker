...
var BadgeUnstyled = React9.forwardRef(function BadgeUnstyled2(props, ref) {
  const {
    component,
    children,
    max: maxProp = 99,
...
var ButtonUnstyled = React11.forwardRef(function ButtonUnstyled2(props, forwardedRef) {
  var _ref;
  const {
    action,
    children,
...
var FormControlUnstyledContext = React12.createContext(void 0);
if (true) {
...
var FormControlUnstyled = React13.forwardRef(function FormControlUnstyled2(props, ref) {
  var _ref;
  const {
    defaultValue,
    children,
  ...
  return (0, import_jsx_runtime14.jsx)(FormControlUnstyledContext_default.Provider, {
    value: childContext,
...
function useFormControlUnstyledContext() {
  return React14.useContext(FormControlUnstyledContext_default);
}
...
var InputUnstyled = React16.forwardRef(function InputUnstyled2(props, forwardedRef) {
...
var MenuUnstyledContext = React17.createContext(null);
...
function useControllableReducer(internalReducer, externalReducer, props) {
  ...
  const [nextState, dispatch] = React18.useReducer(combinedReducer, initialState);
  const dispatchWithProps = React18.useCallback((action) => {
    dispatch(_extends({
      props: props.current
    }, action));
  }, [dispatch, props]);
  const previousState = React18.useRef(initialState);
  React18.useEffect(() => {
...
function useListbox(props) {
  ...
  const id = useId(idProp);
  const defaultIdGenerator = React21.useCallback((_, index) => `${id}-option-${index}`, [id]);
  ...
  const setSelectedValue = React21.useCallback((values) => {
    dispatch({
      type: ActionTypes.setValue,
      event: null,
      value: values
  ...
  const createHandleOptionClick = React21.useCallback((option, other) => (event) => {
    var _other$onClick;
  ...
  const createHandleOptionPointerOver = React21.useCallback((option, other) => (event) => {
  ...
  const getOptionState = React21.useCallback((option) => {
  ...
  const getOptionTabIndex = React21.useCallback((optionState) => {
    if (focusManagement === "activeDescendant") {
      return void 0;
    }
  ...
  const getOptionProps = React21.useCallback((option, otherHandlers = {}) => {
...
function useMenuChangeNotifiers() {
  ...
  const notifyHighlightChanged = React23.useCallback((newValue) => {
...
function useMenu(parameters = {}) {
  ...
  const getItemState = React24.useCallback((id) => {
    const {
      disabled,
      highlighted
    } = getOptionState(id);
...
var OptionGroupUnstyled = React30.forwardRef(function OptionGroupUnstyled2(props, ref) {
  const {
    component,
    disabled = false,
    slotProps = {},
...
var SelectUnstyledContext = React31.createContext(void 0);
...
var OptionUnstyled = React33.forwardRef(function OptionUnstyled2(props, ref) {
  const {
    children,
    component,
    disabled = false,
...
var OptionUnstyled_default = React33.memo(OptionUnstyled);
...
function useSelectChangeNotifiers() {
  const messageBus = useMessageBus();
  const notifySelectionChanged = React34.useCallback((newValue) => {
    messageBus.publish(SELECTION_CHANGE_TOPIC, newValue);
  }, [messageBus]);
  const notifyHighlightChanged = React34.useCallback((newValue) => {
    messageBus.publish(HIGHLIGHT_CHANGE_TOPIC2, newValue);
  }, [messageBus]);
  const registerSelectionChangeHandler = React34.useCallback((handler) => {
    return messageBus.subscribe(SELECTION_CHANGE_TOPIC, handler);
  }, [messageBus]);
  const registerHighlightChangeHandler = React34.useCallback((handler) => {
...
function useSelect(props) {
  ...
  const optionsMap = React35.useMemo(() => {
  ...
  const createHandleListboxItemClick = React35.useCallback((otherHandlers) => (event) => {
    var _otherHandlers$onClic2;
  ...
  const listboxReducer = React35.useCallback((state, action) => {
    const newState = defaultListboxReducer(state, action);
    switch (action.type) {
      case ActionTypes.keyDown:
  ...
  const optionValues = React35.useMemo(() => options.map((o) => o.value), [options]);
  const isOptionDisabled = React35.useCallback((valueToCheck) => {
  ...
  const getOptionProps = React35.useCallback((optionValue, otherHandlers = {}) => {
...
function getOptionsFromChildren(children) {
  ...
  React36.Children.forEach(children, (node) => {
...
var _excluded25 = ["children", "className", "color", "component", "fontSize", "htmlColor", "inheritViewBox", "titleAccess", "viewBox"];
...
var SvgIcon = React53.forwardRef(function SvgIcon2(inProps, ref) {
...
@mui/base/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-7QAXQHVQ.js.map
